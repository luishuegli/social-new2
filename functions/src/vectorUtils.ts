...
const ARCHETYPE_INDICES: { [key: string]: number } = {
  'creator': 0,
  'explorer': 1,
  'organizer': 2,
  'participant': 3
...
const TEMPO_INDICES: { [key: string]: number } = {
  'one-on-one': 4,
  'small-group': 5,
  'large-group': 6
};
...
const INTENT_INDICES: { [key: string]: number } = {
  'spontaneous': 7,
  'planned': 8,
  'both': 9
};
...
export function generateVectorFromDna(dna: any): number[] {
  ...
  if (dna.coreInterests && Array.isArray(dna.coreInterests)) {
    dna.coreInterests.forEach((interest: any) => {
