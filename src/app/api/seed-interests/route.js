...
export async function POST() {
  try {
    ...
    const existingInterests = await db.collection('interests').get();
    if (!existingInterests.empty) {
      console.log('✅ Interests already exist, skipping seed');
      return NextResponse.json({ message: 'Interests already exist', count: existingInterests.size });
    }
    ...
    return NextResponse.json({ message: 'Interests seeded successfully', count });
  } catch (error) {
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ interests: interestsData, count: interestsData.length });
  } catch (error) {
