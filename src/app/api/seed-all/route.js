  ...
  try {
    ...
    for (const g of groups) {
      for (let p = 0; p < postsPerGroup; p++) {
        ...
        const chosenAuthenticity = authenticity[rand(authenticity.length)];
        // Bias towards collaborative for group-centric vibe
        const chosenPostType = (rand(10) < 4) ? 'Collaborative' : 'Individual';
        const baseDoc = {
          activityTitle: `Post ${p+1} in ${g.groupName}`,
          authorId: author.id,
          groupId: g.id,
          groupName: g.groupName,
          authenticityType: chosenAuthenticity,
          postType: chosenPostType,
          media: [],
          description: 'Autogenerated post content to populate the feed.',
          timestamp: FieldValue.serverTimestamp(),
        };
        // Social proof participants (3-6 random members) for collaborative posts
        if (chosenPostType === 'Collaborative') {
          const memberIds = (g.members || []).slice(0, 24);
          const count = 3 + rand(4);
          const participants = Array.from({ length: count }).map(() => {
            const u = users[rand(users.length)];
            return { name: u.displayName, avatarUrl: u.profilePictureUrl };
          });
          postsBatch.set(postRef, { ...baseDoc, participants });
        } else {
          postsBatch.set(postRef, baseDoc);
        }
      }
    }
    await postsBatch.commit();

    // 3b) Seed additional INDIVIDUAL posts (not tied to a group)
    const extraIndividual = Math.max(5, Math.min(50, Number(body?.individualPosts) || 20));
    const extraBatch = adminDb.batch();
    for (let i = 0; i < extraIndividual; i++) {
      const author = users[rand(users.length)];
      ...
      const sampleImages = [
        ...
        'https://images.unsplash.com/photo-1514519022285-0e2e4dbf2296?w=1200&auto=format&fit=crop',
      ];
      const img = sampleImages[rand(sampleImages.length)];
      ...
      extraBatch.set(ref, {
        ...
        authenticityType: chosenAuthenticity,
        postType: 'Individual',
        description: 'Autogenerated individual post to enrich the feed.',
        imageUrl: img,
        media: img ? [{ url: img, type: 'image' }] : [],
        timestamp: FieldValue.serverTimestamp(),
      });
    }
